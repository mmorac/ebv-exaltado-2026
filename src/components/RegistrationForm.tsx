...
const translations = {
  ...
  en: {
    ...
    cellPhone: "Mobile Phone * (Will receive SMS)",
    email: "Email Address",
  ...
  pt: {
    ...
    cellPhone: "Celular * (Receberá SMS)",
    email: "E-mail",
    ...
    <div className="max-w-3xl mx-auto bg-white rounded-2xl md:rounded-[2rem] shadow-2xl overflow-hidden border-4 border-violet-100 my-4 md:my-8 animate-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="bg-gradient-to-r from-violet-700 to-violet-600 p-6 md:p-8 flex items-center justify-between text-white relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
            <button onClick={onBack} className="hover:bg-white/20 p-2 rounded-full transition-colors" type="button">
      ...
      <form onSubmit={handleSubmit} className="p-4 md:p-8 space-y-8 md:space-y-10">
        ...
        <div className="pt-8">
          <Button 
            type="submit" 
            fullWidth 
            className={`text-lg md:text-xl py-4 md:py-5 shadow-[0_8px_0_rgb(14,165,233)] active:shadow-none active:translate-y-2 border-4 border-white ${(calculatedGroup && spotsLeft === 0) || isSubmitting ? 'opacity-50 cursor-not-allowed bg-slate-400 hover:bg-slate-400 shadow-none' : ''}`}
            disabled={(calculatedGroup !== null && spotsLeft === 0) || isSubmitting}
          >
             {isSubmitting ? (
